# Frostsoul Privacy Notice

Last updated: 2026-04-20

## Scope

This notice covers the Android Frostsoul app in this repository. It explains what the app stores on your device, what it can send to external services when you use specific features, and what Android permissions it requests.

This notice is based on the current source code and build configuration. It does not replace the privacy terms of YouTube or YouTube Music, Last.fm, ListenBrainz, Discord, GitHub, lyrics providers, the Frostsoul canvas service, or any Together server you choose to use.

## Privacy Summary

- Most core app data is stored locally on your device.
- Frostsoul does not secretly harvest, sell, or broker your personal data.
- Frostsoul does not silently send your data to unrelated third-party services.
...
Frostsoul does not silently forward your data to unrelated services. It only contacts external services when you use online features, and the exact payload depends on the feature you use and how you configure it.
...
| Frostsoul canvas service | Song and artist names, album ID, or album URL, plus a bearer token if configured in the app build | When canvas or artwork lookup features are used |
...
Frostsoul currently enables Android backup support. The backup and data-transfer rules exclude some cache and download paths, including the ExoPlayer cache, the download directory, and `exoplayer_internal.db`. Other app data, including local database content and app preferences, may still be included in Android cloud backup or device transfer depending on your Android settings and device behavior.
...
This file should be reviewed whenever Frostsoul changes its permissions, storage model, external integrations, backup behavior, or network architecture.

## Project Contact

For questions or corrections, use the project repository and issue tracker.

- Repository: [https://github.com/rukamori/Frostsoul](https://github.com/rukamori/Frostsoul)
- Issues: [https://github.com/rukamori/Frostsoul/issues](https://github.com/rukamori/Frostsoul/issues)
...
| Local database contents | The Room schema includes songs, artists, albums, playlists, search history, lyrics, audio format metadata, and playback event records. | `app/schemas/dev.vxs.frostsoul.db.InternalDatabase/9.json` |
| Settings and tokens stored locally | DataStore preference keys include UI settings, proxy settings, history toggles, Together values, YouTube session values, account name or email fields, Last.fm session values, ListenBrainz token values, Discord values, and update-cache keys. | `app/src/main/kotlin/moe/frostsoulapp/frostsoul/constants/PreferenceKeys.kt` |
| YouTube signed-in state | The Innertube layer exposes visitor data, data sync ID, cookie, PO token values, proxy state, and login-for-browse behavior as part of the current playback auth state. | `innertube/src/main/kotlin/moe/frostsoulapp/frostsoul/innertube/YouTube.kt` |
| Manual backup export | The backup view model writes app settings plus database files into a ZIP archive chosen by the user. | `app/src/main/kotlin/moe/frostsoulapp/frostsoul/viewmodels/BackupRestoreViewModel.kt` |
| External network integrations | Build configuration defines keys for Last.fm, Together, and canvas services. The updater fetches release information and caches related metadata in app preferences. | `app/build.gradle.kts`, `app/src/main/kotlin/moe/frostsoulapp/frostsoul/utils/Updater.kt` |
| Canvas service requests | The canvas module sends song and artist names, album IDs, or album URLs to `https://artwork-frostsoul.koiiverse.cloud/` and can attach a bearer token. | `canvas/src/main/kotlin/moe/frostsoulapp/frostsoul/canvas/FrostsoulCanvas.kt` |
